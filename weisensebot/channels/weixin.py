...
from weisensebot.config.paths import get_media_dir, get_runtime_subdir
from weisensebot.config.schema import Base, WeixinConfig
from weisensebot.utils.helpers import split_message
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'weisensebot weixin login' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"weisensebot-{uuid.uuid4().hex[:12]}"
